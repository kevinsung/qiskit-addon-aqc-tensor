...
With this Qiskit addon, you can compress the *initial portion* of a circuit
...
In order for AQC to be useful on quantum hardware, we must use the
...
AQC-Tensor requires three things as input:
...
1. A description of the **target state** in the form of a tensor
   ...
   did), or it could be generated in some other way, for example, by
...
We then use an iterative optimization method to make the *good* state as close to the target state as possible.
...
1. The **target state**, as in, a *great* description of the desired
   state; and,
...
the ``evolution_time`` is fixed, such that
the time per single Trotter step
...
per Trotter step, ``dt``.) Therefore, to apply AQC-Tensor to a new
physical model of Trotter circuit, you would only need to generalize a
single function with the details of that model.

The ansatz used by
``generate_ansatz_from_circuit`` uses nine parameters per
two-qubit block, which is the theoretical minimum (see, for example, Fig. 2 of
...
circuit will therefore result in three parameters for the two-qubit rotation, plus three
parameters for an outgoing single-qubit rotation on each of the two
qubits, for a total of nine parameters per block.
